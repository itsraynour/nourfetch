        ...
        if output.status.success() {
            ...
            for line in text.lines() {
                ...
                if parts.len() >= 4 {
                    ...
                    gpu.total_vram_bytes = total_mb * 1024 * 1024;
                    gpu.used_vram_bytes = used_mb * 1024 * 1024;
                    gpu.free_vram_bytes = gpu.total_vram_bytes.saturating_sub(gpu.used_vram_bytes);
                    if let Ok(util) = parts[3].parse::<f32>() {
                        gpu.usage_percent = Some(util.min(100.0));
                    } else if gpu.total_vram_bytes > 0 {
                        let pct = (gpu.used_vram_bytes as f32 / gpu.total_vram_bytes as f32) * 100.0;
                        gpu.usage_percent = Some(pct.min(100.0));
                    }
                    if parts.len() >= 5 {
                        gpu.driver_version = parts[4].to_string();
                    }
                    gpus.push(gpu);
                }
            }
                ...
                for entry in entries.filter_map(Result::ok) {
                    ...
                    if file_name.starts_with("card") && !file_name.contains('-') {
                        ...
                        if device_path.exists() {
                            ...
                            if let Ok(uevent) = fs::read_to_string(device_path.join("uevent")) {
                                for line in uevent.lines() {
                                    if let Some((k, v)) = line.split_once('=') {
                                        if k == "DRIVER" {
                                            gpu.vendor = match v {
                                                "amdgpu" | "radeon" => "AMD".to_string(),
                                                "i915" | "xe" => "Intel".to_string(),
                                                "nouveau" | "nvidia" => "NVIDIA".to_string(),
                            ...
                            if let Ok(vram_tot_str) =
                                fs::read_to_string(device_path.join("mem_info_vram_total"))
                            {
                                gpu.total_vram_bytes = vram_tot_str.trim().parse().unwrap_or(0);
                            }
                            if let Ok(vram_used_str) =
                                fs::read_to_string(device_path.join("mem_info_vram_used"))
                            {
                                gpu.used_vram_bytes = vram_used_str.trim().parse().unwrap_or(0);
                                if gpu.total_vram_bytes > 0 {
                                    gpu.free_vram_bytes =
                                        gpu.total_vram_bytes.saturating_sub(gpu.used_vram_bytes);
                                    let pct = (gpu.used_vram_bytes as f32
                                        / gpu.total_vram_bytes as f32)
                                        * 100.0;
                                    gpu.usage_percent = Some(pct.min(100.0));
                                }
                            }
                                ...
                                fs::read_to_string(device_path.join("gpu_busy_percent"))
                            {
                                if let Ok(busy) = busy_str.trim().parse::<f32>() {
                                    gpu.usage_percent = Some(busy.min(100.0));
                                }
                            }

                            let name = if !gpu.vendor.is_empty() {
                                format!("{} GPU ({})", gpu.vendor, file_name)
                            } else {
                                format!("GPU ({})", file_name)
                            };
                            gpu.name = name;
                        ...
                        || line.contains("3D controller:")
                    {
                        if let Some((_, model)) = line.split_once(": ") {
                            ...
                            if gpu.name.contains("NVIDIA") {
                                gpu.vendor = "NVIDIA".to_string();
                            } else if gpu.name.contains("AMD") || gpu.name.contains("ATI") {
                                gpu.vendor = "AMD".to_string();
                            } else if gpu.name.contains("Intel") {
